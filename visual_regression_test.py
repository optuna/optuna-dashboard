...
def create_optuna_storage() -> optuna.storages.InMemoryStorage:
    ...
    study = optuna.create_study(study_name="multi-objective", storage=storage, directions=["minimize", "minimize"])
...
async def take_screenshots(study_ids) -> None:
    ...
    for study_id in study_ids:
        ...
        time.sleep(5)
        await page.screenshot({"path": os.path.join(output_dir, f"study-{study_id}.png")})
    await browser.close()
