...
export const StudyDetail: FC<{
  ...
  useEffect(() => {
    ...
    if ((!isPreferential && page === "trialList") || (isPreferential && page === "top")) {
      if (nTrials < 100) {
        interval = 2000;
      } else if (nTrials < 500) {
        interval = 5000;
  ...
  if (page === "top") {
    ...
    content = (
      ...
      >
        ...
        <StudyNote studyId={studyId} latestNote={studyDetail.note} cardSx={{ flexGrow: 1 }} />
      </Box>
    );
  } else if (page === "graph") {
    content = (
  ...
  return (
    <Box sx={{ display: "flex" }}>
      <AppDrawer studyId={studyId} page={page} toggleColorMode={toggleColorMode} toolbar={toolbar}>
        {content}
      </AppDrawer>
    </Box>
  );
