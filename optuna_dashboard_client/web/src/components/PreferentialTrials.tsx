...
const SettingsPage: FC<{
  ...
  return (
    <Dialog open={settingShown} onClose={onClose} maxWidth="sm" fullWidth={true}>
      ...
      >
        ...
        {outputComponentType === "artifact" ? (
          <FormControl component="fieldset" disabled={studyDetail.union_user_attrs.length === 0}>
            <FormLabel component="legend">
              User Attribute Key Corresponding to Output Artifact Id:
            </FormLabel>
            <Select
              value={studyDetail.union_user_attrs.length !== 0 ? artifactKey ?? "" : "error"}
              onChange={(e) => {
      ...
      <DialogActions>
        ...
        <Button
          onClick={onApply}
          color="primary"
          disabled={outputComponentType === "artifact" && artifactKey === undefined}
        >
          Apply
        </Button>
      </DialogActions>
...
const isComparisonReady = (trial: Trial, componentType: FeedbackComponentType): boolean => {
  if (componentType === undefined || componentType.output_type === "note") {
    return trial.note.body !== "";
  }
  if (componentType.output_type === "artifact") {
    const artifactId = trial?.user_attrs.find((a) => a.key === componentType.artifact_key)?.value;
...
export const getArtifactUrlPath = (
  ...
  return `${import.meta.env.VITE_API_ENDPOINT}/artifacts/${studyId}/${trialId}/${artifactId}`;
};
...
const PreferentialTrial: FC<{
...
}> = ({ trial, studyDetail, candidates, hideTrial, openDetailTrial, openThreejsArtifactModal }) => {
  ...
  const artifactId =
    componentType.output_type === "artifact"
      ? trial?.user_attrs.find((a) => a.key === componentType.artifact_key)?.value
      : undefined;
  const artifact = trial?.artifacts.find((a) => a.artifact_id === artifactId);
  const urlPath =
    ...
    >
      <CardActions>
        ...
        >
          ...
          {componentType.output_type === "artifact" && artifact !== undefined ? (
            <Typography
              variant="h6"
              sx={{
                margin: theme.spacing(0, 2),
...
export const PreferentialTrials: FC<{ studyDetail: StudyDetail | null }> = ({ studyDetail }) => {
  const theme = useTheme();
  const action = actionCreator();
  ...
  const activeTrials = studyDetail.trials.filter(
    (t) => (t.state === "Running" || t.state === "Complete") && !hiddenTrials.has(t.number)
  );
  const newTrials = activeTrials.filter(
    (t) => !displayTrials.display.includes(t.number) && !displayTrials.clicked.includes(t.number)
  );
  ...
  const latestHistoryId =
    studyDetail?.preference_history?.filter((h) => !h.is_removed).pop()?.id ?? null;
          ...
          >
            ...
            >
              ...
              <TrialListDetail
                ...
                isBestTrial={(trialId) =>
                  studyDetail.trials.find((t) => t.trial_id === trialId)?.state === "Complete" ??
                  false
                }
                directions={[]}
                objectiveNames={[]}
