...
async def take_screenshot(browser: Browser, url: str, path: str):
...
async def integration_test_main():
    browser = await launch()
    await take_screenshot(browser, f'http://{host}:{port}/dashboard/', 'study_list.png')
    await take_screenshot(browser, f'http://{host}:{port}/dashboard/studies/0', 'study_detail_0.png')
    await take_screenshot(browser, f'http://{host}:{port}/dashboard/studies/1', 'study_detail_1.png')
    await browser.close()
